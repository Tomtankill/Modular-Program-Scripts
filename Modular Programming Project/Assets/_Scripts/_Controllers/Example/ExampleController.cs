    ...
    void FixedUpdate() {
        ...
        foreach (ContactPoint[] contacts in contactPoints.Values)
            for (int i = 0; i < contacts.Length; i++)
                if (contacts[i].point.y <= selfRigidBody.position.y - bottomCapsuleSphereOrigin && Physics.Raycast(contacts[i].point + Vector3.up, Vector3.down, out hit, 1.1f, ~0) && Vector3.Angle(hit.normal, Vector3.up) <= MaximumSlope) {
                    grounded = true;
        ...
        if (grounded && doJump != 3) {
            ...
            float difference = length - selfRigidBody.velocity.magnitude;
            ...
            if (Mathf.Approximately(difference, 0.0f))
                ...
                movement = new Vector3((targetSpeed.x - selfRigidBody.velocity.x) * difference * acceleration, (targetSpeed.y - selfRigidBody.velocity.y) * difference * acceleration, (targetSpeed.z - selfRigidBody.velocity.z) * difference * acceleration);
            }

            if (doJump == 1) {
                // jump button was pressed, do jump      
                movement.y = JumpSpeed - selfRigidBody.velocity.y;
                doJump = 2;
            ...
            selfRigidBody.AddForce(new Vector3(movement.x, movement.y, movement.z), ForceMode.VelocityChange);
            ...
            if (groundedLastFrame && doJump != 3 && !falling) {
                ...
                if (Physics.Raycast(transform.position, Vector3.down, out hit, fudgeCheck + (selfRigidBody.velocity.magnitude * Time.deltaTime), ~0) && Vector3.Angle(hit.normal, Vector3.up) <= MaximumSlope) {
                    groundedLastFrame = true;
                    ...
                    if (Physics.Raycast(new Vector3(transform.position.x, transform.position.y - bottomCapsuleSphereOrigin, transform.position.z), -hit.normal, out hit, hit.distance, ~0)) {
                        selfRigidBody.AddForce(hit.normal * -hit.distance, ForceMode.VelocityChange);
            ...
            fallSpeed = selfRigidBody.velocity.y;
            ...
            if (!Mathf.Approximately(inputX + inputY, 0.0f)) {
                ...
                float a = movement.x + selfRigidBody.velocity.x;
                float b = movement.z + selfRigidBody.velocity.z;
                ...
                if (length > 0.0f) {
                    if (length > MovementSpeed) {
                        ...
                        length = 1.0f / Mathf.Sqrt(selfRigidBody.velocity.x * selfRigidBody.velocity.x + selfRigidBody.velocity.z * selfRigidBody.velocity.z);
                        Vector3 rigidbodyDirection = new Vector3(selfRigidBody.velocity.x * length, 0.0f, selfRigidBody.velocity.z * length);
                    ...
                    selfRigidBody.AddForce(new Vector3(movement.x, 0.0f, movement.z), ForceMode.VelocityChange);
                }
            }
